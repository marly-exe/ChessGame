...
public abstract class ChessGame {
    ...
    public boolean isPawnPromotion(Square fromSquare, Square toSquare) {
        if (toSquare.getRank() == BoardRank.FIRST || toSquare.getRank() == BoardRank.EIGHTH) {
//            Player playerPromoting = toSquare.getRank() == BoardRank.EIGHTH ? Player.WHITE : Player.BLACK;
            ArrayList<Square> allValidSquares = getAllValidMovesFromSquare(fromSquare);
            for (Square s : allValidSquares) {
                if (s.getFile()==toSquare.getFile() && s.getRank()==toSquare.getRank()) {
                    return true;
                }
            }
        }
